...
class GroupOfBlocks(nn.Module):
    ...
    def __init__(
        ...
        other_blocks = [
            ResNetBlock(
                out_channels, out_channels, norm=norm, elems=elems, dropout=dropout
            )
...
class GroupOfResidualBlocks(nn.Module):
    def __init__(self, num_channels: int, num_blocks: int, use_batchnorm: bool = False):
        super(GroupOfResidualBlocks, self).__init__()
        self.blocks = nn.Sequential(
            *[ResidualBlock(num_channels, use_batchnorm) for _ in range(num_blocks)]
        )
